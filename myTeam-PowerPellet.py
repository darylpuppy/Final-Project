...
class SmartAgent(CaptureAgent):
	...
	def getFeasiblePellets(self, gameState, distance):
		...
		for pellet in pellets:
			if (not enemyStates[0].scaredTimer > 0  and not enemyStates[1].scaredTimer > 0): # if not scared (normal state)
				if self.findDepth(pellet, self.offenseCost, gameState) * 2 + 1 > self.closeMazeEnemy:	#If the pellet is too deep, remove it from the list
					copy.remove(pellet)
			elif (self.distancer.getDistance(myPos,self.enemyPos[0]) < self.distancer.getDistance(myPos,self.enemyPos[1])): # find the closer ghost 
				if (enemyStates[0].scaredTimer <= self.findDepth(pellet, self.offenseCost, gameState) * 2 + 1 ): # if enemy will be scared longer than we can get out of the deadend
					copy.remove(pellet)

			else:
				if (enemyStates[1].scaredTimer <= self.findDepth(pellet, self.offenseCost, gameState) * 2 + 1 ): # if enemy will be scared longer than we can get out of the deadend
					copy.remove(pellet)
...
class SwitchAgent(SmartAgent):
	...
	def setOffenseDefense(self, gameState):
		...
		elif enemyStates[0].scaredTimer > 0 and enemyStates[1].scaredTimer > 0: # if both enemies are scared, we are safe, go get food
	...
	def offenseCost(self, position, gameState):
		...
		if self.isHome(closeEnemy) and self.distFromHome(closeEnemy) > dist:
			...
			if (enemyStates[closeEnemyIndex].scaredTimer > 0):
				enemyCosts = [0, 0, 2, 5, 100] # if enemies are scared they are not very costly 
			else:
...
class ThiefAgent(SmartAgent):
	...
	def isGoal(self, position, gameState):
		...
		if (enemyStates[tempIndex].scaredTimer > depth*2+1): # if enemy will be scared longer than we can get out of the deadend
