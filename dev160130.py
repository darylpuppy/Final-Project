...
class SmartAgent(CaptureAgent):
    actions = {"Stop": (0, 0), "North": (0, 1), "East": (1, 0), "South": (0, -1),
               "West": (-1, 0)}  # All the actions a Pacman can take and how it moves the Pacman
    ...
    def registerInitialState(self, gameState):
        ...
        self.opponentIndices = CaptureAgent.getOpponents(self,
    ...
    def actionPrep(self, gameState):
        self.enemyPos = (gameState.getAgentPosition(self.opponentIndices[0]),
                         gameState.getAgentPosition(self.opponentIndices[1]))  # Get the position of each enemy
        self.enemyDist = [util.manhattanDistance(gameState.getAgentPosition(self.index), self.enemyPos[i]) for i in
                          range(2)]  # Get the manhattan distance to each enemy
    ...
    def aStarSearch(self, gameState, isGoal, heuristic, costFunction):
        ...
        while not queue.isEmpty():  # Continue looking until we have found the goal or tried looking everywhere
            ...
            if isGoal(curPosition[0],
                      gameState):  # If the current state is the goal state, we can exit and move on to generating the path
                break
                ...
                for node in successors:
                    if node[0] not in visited:
                        if (node[0] not in path) or (path[node[0]][2] > (curPosition[1] + node[2])):
                            path[node[0]] = [curPosition[0], node[1], curPosition[1] + node[
                                2]]  # Mark down which state we were in before reaching this one and which action was taken to reach it
                        queue.push([node[0], curPosition[1] + node[2]],
                                   curPosition[1] + node[2] + heuristic(node[0], gameState, position))
    ...
    def getFeasiblePellets(self, gameState, distance):
        ...
        pellets.extend(self.getCapsules(gameState))
        capsules = self.getCapsules(gameState)
        # print pellets
        copy = self.getFood(gameState).asList()  # Allows us to remove pellets without messing up the for loop
        copy.extend(self.getCapsules(gameState))

        # if (enemyStates[0].scaredTimer > 0):
        # pellets.append (self.enemyPos[i])
        # copy.append (self.enemyPos[i])
        # print pellets

        # if ghost is scared, pacman will go into dead end, even if the pellet is too deep
        myPos = gameState.getAgentPosition(self.index)
        for pellet in pellets:
            if (not enemyStates[0].scaredTimer > 0 and not enemyStates[
    ...
    def findDepth(self, position, costFunction, gameState):
        ...
        if (position in self.deadEndMap.asList()):
            curState = self.breadthFirstSearch(position, costFunction, gameState)
            depth = self.distancer.getDistance(position, curState)
    ...
    def breadthFirstSearch(self, position, costFunction, gameState):
        ...
        while not queue.isEmpty():  # Continue looking until we have found the goal or tried looking everywhere
            ...
            for node in successors:
                if node[0] not in visited:
                    path[node[0]] = [curState, node[
                        1]]  # Mark down which state we were in before reaching this one and which action was taken to reach it
                    queue.push(node[0])
            ...
            for x in range(walls.width):
                for y in range(walls.height):
                    ...
                    if (not deadEnds[x][y]):  # If the position isn't a dead end, check if it should be
                        ...
                        if ((isDeadEnd1 and isDeadEnd2 and isDeadEnd3) or (
                                isDeadEnd1 and isDeadEnd2 and isDeadEnd4) or (
                                isDeadEnd1 and isDeadEnd3 and isDeadEnd4) or (
                                isDeadEnd2 and isDeadEnd3 and isDeadEnd4)):
                            deadEnds[x][y] = True
                            done = False
    ...
    def getSuccessors(self, position, walls, costFunction, gameState):
        ...
        for action in self.actions:  # Loop through all legal actions
            nextPos = (position[0] + self.actions[action][0],
                       position[1] + self.actions[action][1])  # Find out where the Pacman would be
            if nextPos not in walls:  # Make sure the Pacman wouldn't move into a wall
                nextStates.append((nextPos, action, costFunction(nextPos, gameState)))
        return nextStates
...
class SwitchAgent(SmartAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        self.start = gameState.getAgentPosition(
            self.index)  # The position we started in, used to determine if we were killed
    ...
    def setOffenseDefense(self, gameState):
        ...
        enemyStates = [gameState.getAgentState(self.opponentIndices[i]) for i in
                       range(2)]  # The states of both enemy teams

        if myState.scaredTimer > 0:  # If we are scared,
            SwitchAgent.onOffense = [True, True]  # So assign both Pacmans to offense
            SwitchAgent.marked = [None, None]  # Because they're on offense, none is marked
        elif enemyStates[0].scaredTimer > 0 and enemyStates[
            1].scaredTimer > 0:  # if both enemies are scared, we are safe, go get food
            SwitchAgent.onOffense = [True, True]
            SwitchAgent.marked = [None, None]  # we are not going to eat them
        elif enemyStates[0].numCarrying > 5 or enemyStates[
            1].numCarrying > 5:  # If an enemy is carrying more than 5 pellets, they are a significant threat
            SwitchAgent.onOffense = [False, False]  # So make both agents target them
        elif self.isHome(pos1) and self.isHome(pos2) and self.getScore(gameState) > .2 * len(self.feasiblePellets):
            SwitchAgent.onOffense = [False, False]
        elif SwitchAgent.onOffense[0] == SwitchAgent.onOffense[
            1]:  # If both agents are on either offense or defense, we need to evaluate whether or not it should stay that way
            if self.getScore(gameState) <= 0 or len(
                    self.feasiblePellets) > 0:  # If we're not winning or there are feasible pellets to get, someone needs to go on offense while the other stays on defense
                if self.isHome(pos1) == self.isHome(
                        pos2):  # If both agents are on the same side, determine who should be on offense based on the distance to the closest enemy
                    mazeDists = [min(self.distancer.getDistance(pos1, self.enemyPos[i]) for i in range(2)), min(
                        self.distancer.getDistance(pos2, self.enemyPos[i]) for i in
                        range(2))]  # Get the distance to the closest enemy for each agent
                    if mazeDists[0] > mazeDists[1]:  # If the first agent is closer to an enemy than the second,
                        ...
                        pos1):  # If one agent is on the far side while the other is home, that agent goes on offense
                    SwitchAgent.onOffense = [False, True]
                    SwitchAgent.marked[1] = None
                ...
                SwitchAgent.onOffense = [False, False]
        elif len(self.feasiblePellets) == 0 and SwitchAgent.onOffense[self.teamOrder] and self.getScore(
                gameState) > 0:  # Same as above, but make sure the agent on offense if evaluating whether or not there are any feasible pellets
            SwitchAgent.onOffense = [False, False]
    ...
    def chooseAction(self, gameState):
        ...
        if position == self.goal:  # If we've reached a goal, we are free to choose a new one
            self.goal = None
        self.enemyMazeDists = [self.distancer.getDistance(position, self.enemyPos[i]) for i in
                               range(2)]  # Get the real distance to each enemy
        self.closeMazeEnemy = min(self.enemyMazeDists)  # Get the distance to the closer of the two

        self.feasiblePellets = self.getFeasiblePellets(gameState,
                                                       self.closeMazeEnemy)  # Find all the pellets that can be retrieved without going too deep into a dead end
        self.setOffenseDefense(gameState)  # Set the offense defense arrangement for this turn

        if SwitchAgent.onOffense[self.teamOrder]:  # If this agent is on offense, use the appropriate algorithms
            ...
            if SwitchAgent.onOffense == [False, False] and (self.isHome(self.enemyPos[0]) != self.isHome(self.enemyPos[
                                                                                                             1])):  # If both agents are on defense and exactly one enemy is home, try to cut off that enemy
        ...
        if self.isPellet((position[0] + self.actions[action][0], position[1] + self.actions[action][1]),
                         gameState):  # If our next move takes us into a pellet, we are now carrying at least one
            self.hasPellet = True
    ...
    def offenseCost(self, position, gameState):
        ...
        enemyDist = [util.manhattanDistance(position, self.enemyPos[i]) for i in
                     range(2)]  # Get the manhattan distance from each enemy
        ...
        if self.isHome(closeEnemy) and self.distFromHome(closeEnemy) > dist:
            if gameState.getAgentState(self.index).scaredTimer > 0:
                enemyCosts = [9999, 999, 10, 5, 0]
            else:
                enemyCosts = [-10, -5, -3, -1, 0]  # Costs for getting close to an enemy on the home side
        else:
            enemyStates = [gameState.getAgentState(self.opponentIndices[i]) for i in range(2)]
            if (enemyStates[closeEnemyIndex].scaredTimer > 0):
                enemyCosts = [0, 0, 0, 0, 0]  # if enemies are scared they are not very costly
            else:
                enemyCosts = [9999, 999, 10, 5, 0]  # Costs for getting close to an enemy on the far side
        ...
        if kill:  # If we're trying to kill them, the only goal state is where they are
            ...
            for i in range(height):  # Loop through all legal positions on the boundary
                if (boundary, i) in walls:  # Anywhere that is a wall isn't legal, so we continue
                    continue
                enemyDist = min([util.manhattanDistance((boundary, i), enemyPos[j]) + enemyStates[j].numReturned for j in range(len(enemyPos))])
                if enemyDist < bestDist:  # Compare that to the current closest distance
                    bestSpot = (boundary, i)
                    bestDist = enemyDist
    ...
    def cutoffGoal(self, position, gameState):
        ...
        if self.isHome(self.enemyPos[0]) and self.isHome(self.enemyPos[1]):
            enemyStates = [gameState.getAgentState(self.opponentIndices[i]) for i in range(2)]
            if enemyStates[0].numCarrying > enemyStates[1].numCarrying:
                return position == self.enemyPos[0]
            elif enemyStates[1].numCarrying > enemyStates[0].numCarrying:
                return position == self.enemyPos[1]
    ...
    def cutoffCost(self, position, gameState):
        ...
        if position == positions[
            self.teamOrder * -1 + 1]:  # If the position is the same as a partner, the cost becomes prohibitively high
            cost += 9999
        return cost
